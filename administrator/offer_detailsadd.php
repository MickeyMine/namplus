...
$offer_details_add = NULL; // Initialize page object first

class coffer_details_add extends coffer_details {
	...
	// Page object name
	var $PageObjName = 'offer_details_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["offer_detail_id"] != "") {
				...
				$this->setKey("offer_detail_id", $this->offer_detail_id->CurrentValue); // Set up key
			} else {
				$this->setKey("offer_detail_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "offer_detailsview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$this->offer_detail_id->setDbValue($conn->Insert_ID());
			$rsnew['offer_detail_id'] = $this->offer_detail_id->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($offer_details_add)) $offer_details_add = new coffer_details_add();

// Page init
$offer_details_add->Page_Init();

// Page main
$offer_details_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$offer_details_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var offer_details_add = new ew_Page("offer_details_add");
offer_details_add.PageID = "add"; // Page ID
var EW_PAGE_ID = offer_details_add.PageID; // For backward compatibility

// Form object
var foffer_detailsadd = new ew_Form("foffer_detailsadd");

// Validate form
foffer_detailsadd.Validate = function() {
...
foffer_detailsadd.Form_CustomValidate = 
...
foffer_detailsadd.ValidateRequired = true;
<?php } else { ?>
foffer_detailsadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
foffer_detailsadd.Lists["x_offer_id"] = {"LinkField":"x_offer_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_offer_title","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $offer_details_add->ShowPageHeader(); ?>
<?php
$offer_details_add->ShowMessage();
?>
<form name="foffer_detailsadd" id="foffer_detailsadd" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="offer_details">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_offer_detailsadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
foffer_detailsadd.Lists["x_offer_id"].Options = <?php echo (is_array($offer_details->offer_id->EditValue)) ? ew_ArrayToJson($offer_details->offer_id->EditValue, 1) : "[]" ?>;
</script>
</span>
...
ew_CreateCalendar("foffer_detailsadd", "x_offer_start_date", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
ew_CreateCalendar("foffer_detailsadd", "x_offer_end_date", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
foffer_detailsadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$offer_details_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$offer_details_add->Page_Terminate();
?>
