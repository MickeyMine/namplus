...
function CurrentProjectID() {
	...
	return "{621448A2-A15A-4302-8B90-FC8E171BD28F}";
}
	...
	if ($fn <> "") {
		if ($fld->IsBlobImage || ew_IsImageFile($fn)) {
			...
			if ($fld->FldDataType == EW_DATATYPE_BLOB)
				...
				return ($fld->CurrentValue <> "") ? $fld->CurrentValue : $fld->FldCaption();
		}
	} else {
		return "";
	}
	...
	function ValidateUser(&$usr, &$pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('user_pass'), $pwd);
					if ($ValidateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						...
						if (is_null($rs->fields('privilege_id'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('privilege_id'))); // Load User Level
						}
						$this->SetUpUserLevel();
