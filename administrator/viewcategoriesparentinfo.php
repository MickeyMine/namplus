...
$viewcategoriesparent = NULL;

//
// Table class for viewcategoriesparent
//
class cviewcategoriesparent extends cTable {
	var $cat_id;
	var $cat_name;
	var $cat_description;
	var $cat_parent_id;
	...
	function __construct() {
		...
		$this->TableVar = 'viewcategoriesparent';
		$this->TableName = 'viewcategoriesparent';
		$this->TableType = 'VIEW';
		$this->ExportAll = TRUE;
		...
		// cat_id
		$this->cat_id = new cField('viewcategoriesparent', 'viewcategoriesparent', 'x_cat_id', 'cat_id', '`cat_id`', '`cat_id`', 3, -1, FALSE, '`cat_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->cat_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cat_id'] = &$this->cat_id;

		// cat_name
		$this->cat_name = new cField('viewcategoriesparent', 'viewcategoriesparent', 'x_cat_name', 'cat_name', '`cat_name`', '`cat_name`', 200, -1, FALSE, '`cat_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['cat_name'] = &$this->cat_name;

		// cat_description
		$this->cat_description = new cField('viewcategoriesparent', 'viewcategoriesparent', 'x_cat_description', 'cat_description', '`cat_description`', '`cat_description`', 200, -1, FALSE, '`cat_description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['cat_description'] = &$this->cat_description;

		// cat_parent_id
		$this->cat_parent_id = new cField('viewcategoriesparent', 'viewcategoriesparent', 'x_cat_parent_id', 'cat_parent_id', '`cat_parent_id`', '`cat_parent_id`', 3, -1, FALSE, '`cat_parent_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->cat_parent_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cat_parent_id'] = &$this->cat_parent_id;

		// cat_order
		$this->cat_order = new cField('viewcategoriesparent', 'viewcategoriesparent', 'x_cat_order', 'cat_order', '`cat_order`', '`cat_order`', 3, -1, FALSE, '`cat_order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->cat_order->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cat_order'] = &$this->cat_order;

		// cat_status
		$this->cat_status = new cField('viewcategoriesparent', 'viewcategoriesparent', 'x_cat_status', 'cat_status', '`cat_status`', '`cat_status`', 16, -1, FALSE, '`cat_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->cat_status->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('cat_id', $rs))
				ew_AddFilter($where, ew_QuotedName('cat_id') . '=' . ew_QuotedValue($rs['cat_id'], $this->cat_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "viewcategoriesparentlist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("viewcategoriesparentview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("viewcategoriesparentview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "viewcategoriesparentadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("viewcategoriesparentedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("viewcategoriesparentadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("viewcategoriesparentdelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->cat_status->Exportable) $Doc->ExportCaption($this->cat_status);
			} else {
				if ($this->cat_id->Exportable) $Doc->ExportCaption($this->cat_id);
				if ($this->cat_name->Exportable) $Doc->ExportCaption($this->cat_name);
				if ($this->cat_description->Exportable) $Doc->ExportCaption($this->cat_description);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->cat_status->Exportable) $Doc->ExportField($this->cat_status);
				} else {
					if ($this->cat_id->Exportable) $Doc->ExportField($this->cat_id);
					if ($this->cat_name->Exportable) $Doc->ExportField($this->cat_name);
					if ($this->cat_description->Exportable) $Doc->ExportField($this->cat_description);
