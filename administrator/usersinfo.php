...
class cusers extends cTable {
	...
	function __construct() {
		...
		// user_id
		$this->user_id = new cField('users', 'users', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 3, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// user_pass
		$this->user_pass = new cField('users', 'users', 'x_user_pass', 'user_pass', '`user_pass`', '`user_pass`', 200, -1, FALSE, '`user_pass`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['user_pass'] = &$this->user_pass;

		// privilege_id
		$this->privilege_id = new cField('users', 'users', 'x_privilege_id', 'privilege_id', '`privilege_id`', '`privilege_id`', 3, -1, FALSE, '`privilege_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->privilege_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['privilege_id'] = &$this->privilege_id;

		// user_status
		$this->user_status = new cField('users', 'users', 'x_user_status', 'user_status', '`user_status`', '`user_status`', 16, -1, FALSE, '`user_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('user_id', $rs))
				ew_AddFilter($where, ew_QuotedName('user_id') . '=' . ew_QuotedValue($rs['user_id'], $this->user_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
   ...
   // Common render codes
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->privilege_id->Exportable) $Doc->ExportCaption($this->privilege_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->privilege_id->Exportable) $Doc->ExportField($this->privilege_id);
	...
	function UserIDFilter($userid) {
		$sUserIDFilter = '`user_id` = ' . ew_QuotedValue($userid, EW_DATATYPE_NUMBER);
