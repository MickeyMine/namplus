...
$image_gallery_img_path_blobview = NULL; // Initialize page object first

class cimage_gallery_img_path_blobview extends cimage_gallery {
	...
	// Page object name
	var $PageObjName = 'image_gallery_img_path_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'image_gallery');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('image_gallery', 'x_img_path');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('img_path');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('img_path')));
				if (trim(strval($this->Recordset->fields('img_path'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('img_path') . "\"");
				}
...
// Create page object
if (!isset($image_gallery_img_path_blobview)) $image_gallery_img_path_blobview = new cimage_gallery_img_path_blobview();

// Page init
$image_gallery_img_path_blobview->Page_Init();

// Page main
$image_gallery_img_path_blobview->Page_Main();
?>
<?php
$image_gallery_img_path_blobview->Page_Terminate();
?>
