...
class coffers_edit extends coffers {
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->offer_id->EditValue = $this->offer_id->CurrentValue;
			$this->offer_id->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->offer_image_path);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// offer_title
			$this->offer_title->SetDbValueDef($rsnew, $this->offer_title->CurrentValue, "", $this->offer_title->ReadOnly);

			// offer_description
			$this->offer_description->SetDbValueDef($rsnew, $this->offer_description->CurrentValue, "", $this->offer_description->ReadOnly);

			// offer_content
			$this->offer_content->SetDbValueDef($rsnew, $this->offer_content->CurrentValue, "", $this->offer_content->ReadOnly);

			// offer_question_content
			$this->offer_question_content->SetDbValueDef($rsnew, $this->offer_question_content->CurrentValue, "", $this->offer_question_content->ReadOnly);

			// offer_image_path
			if (!($this->offer_image_path->ReadOnly) && !$this->offer_image_path->Upload->KeepFile) {
				$this->offer_image_path->Upload->DbValue = $rs->fields('offer_image_path'); // Get original value
			...
			// offer_cat_id
			$this->offer_cat_id->SetDbValueDef($rsnew, $this->offer_cat_id->CurrentValue, 0, $this->offer_cat_id->ReadOnly);

			// offer_status
			$this->offer_status->SetDbValueDef($rsnew, $this->offer_status->CurrentValue, 0, $this->offer_status->ReadOnly);
...
// Page object
var offers_edit = new ew_Page("offers_edit");
offers_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = offers_edit.PageID; // For backward compatibility

// Form object
var foffersedit = new ew_Form("foffersedit");
...
foffersedit.Lists["x_offer_cat_id"] = {"LinkField":"x_cat_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_cat_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="foffersedit" id="foffersedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="offers">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_offersedit" class="table table-bordered table-striped">
...
<?php echo $offers->offer_id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_offer_id" name="x_offer_id" id="x_offer_id" value="<?php echo ew_HtmlEncode($offers->offer_id->CurrentValue) ?>">
<?php echo $offers->offer_id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("foffersedit", "x_offer_content", 35, 4, <?php echo ($offers->offer_content->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<script type="text/javascript">
ew_CreateEditor("foffersedit", "x_offer_question_content", 35, 4, <?php echo ($offers->offer_question_content->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php if (@$_POST["fa_x_offer_image_path"] == "0") { ?>
<input type="hidden" name="fa_x_offer_image_path" id= "fa_x_offer_image_path" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_offer_image_path" id= "fa_x_offer_image_path" value="1">
<?php } ?>
...
ew_CreateCalendar("foffersedit", "x_offer_start_date", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
ew_CreateCalendar("foffersedit", "x_offer_end_date", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
foffersedit.Lists["x_offer_cat_id"].Options = <?php echo (is_array($offers->offer_cat_id->EditValue)) ? ew_ArrayToJson($offers->offer_cat_id->EditValue, 1) : "[]" ?>;
</script>
</span>
