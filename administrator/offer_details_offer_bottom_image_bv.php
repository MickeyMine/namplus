...
$offer_details_offer_bottom_image_blobview = NULL; // Initialize page object first

class coffer_details_offer_bottom_image_blobview extends coffer_details {
	...
	// Page object name
	var $PageObjName = 'offer_details_offer_bottom_image_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'offer_details');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('offer_details', 'x_offer_bottom_image');

		// Show thumbnail
		$bShowThumbnail = (@$_GET["showthumbnail"] == "1");
		if (@$_GET["thumbnailwidth"] == "" && @$_GET["thumbnailheight"] == "") {
			$iThumbnailWidth = 80; // Set default width
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('offer_bottom_image');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('offer_bottom_image')));
				if (trim(strval($this->Recordset->fields('offer_bottom_image'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('offer_bottom_image') . "\"");
				}
...
// Create page object
if (!isset($offer_details_offer_bottom_image_blobview)) $offer_details_offer_bottom_image_blobview = new coffer_details_offer_bottom_image_blobview();

// Page init
$offer_details_offer_bottom_image_blobview->Page_Init();

// Page main
$offer_details_offer_bottom_image_blobview->Page_Main();
?>
<?php
$offer_details_offer_bottom_image_blobview->Page_Terminate();
?>
