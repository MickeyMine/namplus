...
$offer_cus_answers_view = NULL; // Initialize page object first

class coffer_cus_answers_view extends coffer_cus_answers {
	...
	// Page object name
	var $PageObjName = 'offer_cus_answers_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["cusans_id"] <> "") {
			$this->RecKey["cusans_id"] = $_GET["cusans_id"];
			$KeyUrl .= "&amp;cusans_id=" . urlencode($this->RecKey["cusans_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["cusans_id"] <> "") {
				...
				$this->RecKey["cusans_id"] = $this->cusans_id->QueryStringValue;
			} else {
				$sReturnUrl = "offer_cus_answerslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "offer_cus_answerslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "offer_cus_answerslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($offer_cus_answers_view)) $offer_cus_answers_view = new coffer_cus_answers_view();

// Page init
$offer_cus_answers_view->Page_Init();

// Page main
$offer_cus_answers_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$offer_cus_answers_view->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var offer_cus_answers_view = new ew_Page("offer_cus_answers_view");
offer_cus_answers_view.PageID = "view"; // Page ID
var EW_PAGE_ID = offer_cus_answers_view.PageID; // For backward compatibility

// Form object
var foffer_cus_answersview = new ew_Form("foffer_cus_answersview");

// Form_CustomValidate event
foffer_cus_answersview.Form_CustomValidate = 
...
foffer_cus_answersview.ValidateRequired = true;
<?php } else { ?>
foffer_cus_answersview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
foffer_cus_answersview.Lists["x_cusans_customer_id"] = {"LinkField":"x_customer_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_customer_code","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
foffer_cus_answersview.Lists["x_cusans_offer_id"] = {"LinkField":"x_offer_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_offer_title","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $offer_cus_answers_view->ExportOptions->Render("body") ?>
<?php if (!$offer_cus_answers_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($offer_cus_answers_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php $offer_cus_answers_view->ShowPageHeader(); ?>
<?php
$offer_cus_answers_view->ShowMessage();
?>
<form name="foffer_cus_answersview" id="foffer_cus_answersview" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="offer_cus_answers">
<table class="ewGrid"><tr><td>
<table id="tbl_offer_cus_answersview" class="table table-bordered table-striped">
...
<?php echo $offer_cus_answers->cusans_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($offer_cus_answers->cusans_customer_id->Visible) { // cusans_customer_id ?>
	<tr id="r_cusans_customer_id">
		<td><span id="elh_offer_cus_answers_cusans_customer_id"><?php echo $offer_cus_answers->cusans_customer_id->FldCaption() ?></span></td>
...
<?php echo $offer_cus_answers->cusans_customer_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($offer_cus_answers->cusans_offer_id->Visible) { // cusans_offer_id ?>
	<tr id="r_cusans_offer_id">
		<td><span id="elh_offer_cus_answers_cusans_offer_id"><?php echo $offer_cus_answers->cusans_offer_id->FldCaption() ?></span></td>
...
<?php echo $offer_cus_answers->cusans_offer_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($offer_cus_answers->cusans_content->Visible) { // cusans_content ?>
	<tr id="r_cusans_content">
		<td><span id="elh_offer_cus_answers_cusans_content"><?php echo $offer_cus_answers->cusans_content->FldCaption() ?></span></td>
...
<span<?php echo $offer_cus_answers->cusans_content->ViewAttributes() ?>>
<?php echo $offer_cus_answers->cusans_content->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
foffer_cus_answersview.Init();
</script>
<?php
$offer_cus_answers_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$offer_cus_answers_view->Page_Terminate();
?>
