...
$image_gallery_edit = NULL; // Initialize page object first

class cimage_gallery_edit extends cimage_gallery {
	...
	// Page object name
	var $PageObjName = 'image_gallery_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->img_id->CurrentValue == "")
			$this->Page_Terminate("image_gallerylist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->img_id->FldIsDetailKey)
			$this->img_id->setFormValue($objForm->GetValue("x_img_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->img_id->CurrentValue = $this->img_id->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// img_id
			$this->img_id->EditCustomAttributes = "";
			$this->img_id->EditValue = $this->img_id->CurrentValue;
			$this->img_id->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_path);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// img_path
			if (!($this->img_path->ReadOnly) && !$this->img_path->Upload->KeepFile) {
				$this->img_path->Upload->DbValue = $rs->fields('img_path'); // Get original value
			...
			// img_description
			$this->img_description->SetDbValueDef($rsnew, $this->img_description->CurrentValue, "", $this->img_description->ReadOnly);

			// img_cat_id
			$this->img_cat_id->SetDbValueDef($rsnew, $this->img_cat_id->CurrentValue, NULL, $this->img_cat_id->ReadOnly);

			// img_new_id
			$this->img_new_id->SetDbValueDef($rsnew, $this->img_new_id->CurrentValue, NULL, $this->img_new_id->ReadOnly);

			// img_offer_id
			$this->img_offer_id->SetDbValueDef($rsnew, $this->img_offer_id->CurrentValue, NULL, $this->img_offer_id->ReadOnly);

			// img_nam_archive
			$this->img_nam_archive->SetDbValueDef($rsnew, $this->img_nam_archive->CurrentValue, 0, $this->img_nam_archive->ReadOnly);

			// img_is_banner
			$this->img_is_banner->SetDbValueDef($rsnew, $this->img_is_banner->CurrentValue, 0, $this->img_is_banner->ReadOnly);

			// img_order
			$this->img_order->SetDbValueDef($rsnew, $this->img_order->CurrentValue, 0, $this->img_order->ReadOnly);

			// img_status
			$this->img_status->SetDbValueDef($rsnew, $this->img_status->CurrentValue, 0, $this->img_status->ReadOnly);
...
// Create page object
if (!isset($image_gallery_edit)) $image_gallery_edit = new cimage_gallery_edit();

// Page init
$image_gallery_edit->Page_Init();

// Page main
$image_gallery_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$image_gallery_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var image_gallery_edit = new ew_Page("image_gallery_edit");
image_gallery_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = image_gallery_edit.PageID; // For backward compatibility

// Form object
var fimage_galleryedit = new ew_Form("fimage_galleryedit");

// Validate form
fimage_galleryedit.Validate = function() {
...
fimage_galleryedit.Form_CustomValidate = 
...
fimage_galleryedit.ValidateRequired = true;
<?php } else { ?>
fimage_galleryedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fimage_galleryedit.Lists["x_img_cat_id"] = {"LinkField":"x_cat_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_cat_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fimage_galleryedit.Lists["x_img_new_id"] = {"LinkField":"x_new_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_new_title","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fimage_galleryedit.Lists["x_img_offer_id"] = {"LinkField":"x_offer_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_offer_title","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $image_gallery_edit->ShowPageHeader(); ?>
<?php
$image_gallery_edit->ShowMessage();
?>
<form name="fimage_galleryedit" id="fimage_galleryedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="image_gallery">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_image_galleryedit" class="table table-bordered table-striped">
...
<?php echo $image_gallery->img_id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_img_id" name="x_img_id" id="x_img_id" value="<?php echo ew_HtmlEncode($image_gallery->img_id->CurrentValue) ?>">
<?php echo $image_gallery->img_id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php if (@$_POST["fa_x_img_path"] == "0") { ?>
<input type="hidden" name="fa_x_img_path" id= "fa_x_img_path" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_path" id= "fa_x_img_path" value="1">
<?php } ?>
...
<script type="text/javascript">
fimage_galleryedit.Lists["x_img_cat_id"].Options = <?php echo (is_array($image_gallery->img_cat_id->EditValue)) ? ew_ArrayToJson($image_gallery->img_cat_id->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fimage_galleryedit.Lists["x_img_new_id"].Options = <?php echo (is_array($image_gallery->img_new_id->EditValue)) ? ew_ArrayToJson($image_gallery->img_new_id->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fimage_galleryedit.Lists["x_img_offer_id"].Options = <?php echo (is_array($image_gallery->img_offer_id->EditValue)) ? ew_ArrayToJson($image_gallery->img_offer_id->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fimage_galleryedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$image_gallery_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$image_gallery_edit->Page_Terminate();
?>
