...
$offer_answers_view = NULL; // Initialize page object first

class coffer_answers_view extends coffer_answers {
	...
	// Page object name
	var $PageObjName = 'offer_answers_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["answer_id"] <> "") {
			$this->RecKey["answer_id"] = $_GET["answer_id"];
			$KeyUrl .= "&amp;answer_id=" . urlencode($this->RecKey["answer_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["answer_id"] <> "") {
				...
				$this->RecKey["answer_id"] = $this->answer_id->QueryStringValue;
			} else {
				$sReturnUrl = "offer_answerslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "offer_answerslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "offer_answerslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($offer_answers_view)) $offer_answers_view = new coffer_answers_view();

// Page init
$offer_answers_view->Page_Init();

// Page main
$offer_answers_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$offer_answers_view->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var offer_answers_view = new ew_Page("offer_answers_view");
offer_answers_view.PageID = "view"; // Page ID
var EW_PAGE_ID = offer_answers_view.PageID; // For backward compatibility

// Form object
var foffer_answersview = new ew_Form("foffer_answersview");

// Form_CustomValidate event
foffer_answersview.Form_CustomValidate = 
...
foffer_answersview.ValidateRequired = true;
<?php } else { ?>
foffer_answersview.ValidateRequired = false; 
<?php } ?>
...
<?php $offer_answers_view->ExportOptions->Render("body") ?>
<?php if (!$offer_answers_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($offer_answers_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php $offer_answers_view->ShowPageHeader(); ?>
<?php
$offer_answers_view->ShowMessage();
?>
<form name="foffer_answersview" id="foffer_answersview" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="offer_answers">
<table class="ewGrid"><tr><td>
<table id="tbl_offer_answersview" class="table table-bordered table-striped">
...
<?php echo $offer_answers->answer_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($offer_answers->question_id->Visible) { // question_id ?>
	<tr id="r_question_id">
		<td><span id="elh_offer_answers_question_id"><?php echo $offer_answers->question_id->FldCaption() ?></span></td>
...
<?php echo $offer_answers->question_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($offer_answers->answer_content->Visible) { // answer_content ?>
	<tr id="r_answer_content">
		<td><span id="elh_offer_answers_answer_content"><?php echo $offer_answers->answer_content->FldCaption() ?></span></td>
		<td<?php echo $offer_answers->answer_content->CellAttributes() ?>>
<span id="el_offer_answers_answer_content" class="control-group">
<span<?php echo $offer_answers->answer_content->ViewAttributes() ?>>
<?php echo $offer_answers->answer_content->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($offer_answers->answer_status->Visible) { // answer_status ?>
	<tr id="r_answer_status">
		<td><span id="elh_offer_answers_answer_status"><?php echo $offer_answers->answer_status->FldCaption() ?></span></td>
...
<?php echo $offer_answers->answer_status->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
foffer_answersview.Init();
</script>
<?php
$offer_answers_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$offer_answers_view->Page_Terminate();
?>
