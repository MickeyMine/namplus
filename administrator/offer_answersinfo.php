...
//
// Table class for offer_answers
//
class coffer_answers extends cTable {
	var $answer_id;
	var $question_id;
	var $answer_content;
	var $answer_status;
	...
	function __construct() {
		...
		$this->TableVar = 'offer_answers';
		$this->TableName = 'offer_answers';
		...
		// answer_id
		$this->answer_id = new cField('offer_answers', 'offer_answers', 'x_answer_id', 'answer_id', '`answer_id`', '`answer_id`', 3, -1, FALSE, '`answer_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->answer_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['answer_id'] = &$this->answer_id;

		// question_id
		$this->question_id = new cField('offer_answers', 'offer_answers', 'x_question_id', 'question_id', '`question_id`', '`question_id`', 3, -1, FALSE, '`question_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->question_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['question_id'] = &$this->question_id;

		// answer_content
		$this->answer_content = new cField('offer_answers', 'offer_answers', 'x_answer_content', 'answer_content', '`answer_content`', '`answer_content`', 201, -1, FALSE, '`answer_content`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['answer_content'] = &$this->answer_content;

		// answer_status
		$this->answer_status = new cField('offer_answers', 'offer_answers', 'x_answer_status', 'answer_status', '`answer_status`', '`answer_status`', 16, -1, FALSE, '`answer_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->answer_status->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['answer_status'] = &$this->answer_status;
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('answer_id', $rs))
				ew_AddFilter($where, ew_QuotedName('answer_id') . '=' . ew_QuotedValue($rs['answer_id'], $this->answer_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("offer_answersview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("offer_answersview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "offer_answersadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("offer_answersedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("offer_answersadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("offer_answersdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["answer_id"]; // answer_id
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->answer_id->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->answer_id->Exportable) $Doc->ExportCaption($this->answer_id);
				if ($this->question_id->Exportable) $Doc->ExportCaption($this->question_id);
				if ($this->answer_content->Exportable) $Doc->ExportCaption($this->answer_content);
				if ($this->answer_status->Exportable) $Doc->ExportCaption($this->answer_status);
			} else {
				if ($this->answer_id->Exportable) $Doc->ExportCaption($this->answer_id);
				if ($this->question_id->Exportable) $Doc->ExportCaption($this->question_id);
				if ($this->answer_status->Exportable) $Doc->ExportCaption($this->answer_status);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->answer_id->Exportable) $Doc->ExportField($this->answer_id);
					if ($this->question_id->Exportable) $Doc->ExportField($this->question_id);
					if ($this->answer_content->Exportable) $Doc->ExportField($this->answer_content);
					if ($this->answer_status->Exportable) $Doc->ExportField($this->answer_status);
				} else {
					if ($this->answer_id->Exportable) $Doc->ExportField($this->answer_id);
					if ($this->question_id->Exportable) $Doc->ExportField($this->question_id);
					if ($this->answer_status->Exportable) $Doc->ExportField($this->answer_status);
