...
$news_new_img_path_blobview = NULL; // Initialize page object first

class cnews_new_img_path_blobview extends cnews {
	...
	// Page object name
	var $PageObjName = 'news_new_img_path_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('news', 'x_new_img_path');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('new_img_path');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('new_img_path')));
				if (trim(strval($this->Recordset->fields('new_img_path'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('new_img_path') . "\"");
				}
...
// Create page object
if (!isset($news_new_img_path_blobview)) $news_new_img_path_blobview = new cnews_new_img_path_blobview();

// Page init
$news_new_img_path_blobview->Page_Init();

// Page main
$news_new_img_path_blobview->Page_Main();
?>
<?php
$news_new_img_path_blobview->Page_Terminate();
?>
