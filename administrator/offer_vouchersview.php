...
$offer_vouchers_view = NULL; // Initialize page object first

class coffer_vouchers_view extends coffer_vouchers {
	...
	// Page object name
	var $PageObjName = 'offer_vouchers_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["voucher_id"] <> "") {
			$this->RecKey["voucher_id"] = $_GET["voucher_id"];
			$KeyUrl .= "&amp;voucher_id=" . urlencode($this->RecKey["voucher_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["voucher_id"] <> "") {
				...
				$this->RecKey["voucher_id"] = $this->voucher_id->QueryStringValue;
			} else {
				$sReturnUrl = "offer_voucherslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "offer_voucherslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "offer_voucherslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($offer_vouchers_view)) $offer_vouchers_view = new coffer_vouchers_view();

// Page init
$offer_vouchers_view->Page_Init();

// Page main
$offer_vouchers_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$offer_vouchers_view->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var offer_vouchers_view = new ew_Page("offer_vouchers_view");
offer_vouchers_view.PageID = "view"; // Page ID
var EW_PAGE_ID = offer_vouchers_view.PageID; // For backward compatibility

// Form object
var foffer_vouchersview = new ew_Form("foffer_vouchersview");

// Form_CustomValidate event
foffer_vouchersview.Form_CustomValidate = 
...
foffer_vouchersview.ValidateRequired = true;
<?php } else { ?>
foffer_vouchersview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
foffer_vouchersview.Lists["x_voucher_offer_id"] = {"LinkField":"x_offer_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_offer_title","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $offer_vouchers_view->ExportOptions->Render("body") ?>
<?php if (!$offer_vouchers_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($offer_vouchers_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php $offer_vouchers_view->ShowPageHeader(); ?>
<?php
$offer_vouchers_view->ShowMessage();
?>
<form name="foffer_vouchersview" id="foffer_vouchersview" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="offer_vouchers">
<table class="ewGrid"><tr><td>
<table id="tbl_offer_vouchersview" class="table table-bordered table-striped">
...
<?php echo $offer_vouchers->voucher_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($offer_vouchers->voucher_number->Visible) { // voucher_number ?>
	<tr id="r_voucher_number">
		<td><span id="elh_offer_vouchers_voucher_number"><?php echo $offer_vouchers->voucher_number->FldCaption() ?></span></td>
...
<?php echo $offer_vouchers->voucher_number->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($offer_vouchers->voucher_offer_id->Visible) { // voucher_offer_id ?>
	<tr id="r_voucher_offer_id">
		<td><span id="elh_offer_vouchers_voucher_offer_id"><?php echo $offer_vouchers->voucher_offer_id->FldCaption() ?></span></td>
...
<?php echo $offer_vouchers->voucher_offer_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($offer_vouchers->voucher_status->Visible) { // voucher_status ?>
	<tr id="r_voucher_status">
		<td><span id="elh_offer_vouchers_voucher_status"><?php echo $offer_vouchers->voucher_status->FldCaption() ?></span></td>
...
<?php echo $offer_vouchers->voucher_status->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
foffer_vouchersview.Init();
</script>
<?php
$offer_vouchers_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$offer_vouchers_view->Page_Terminate();
?>
