...
$offers_offer_image_path_blobview = NULL; // Initialize page object first

class coffers_offer_image_path_blobview extends coffers {
	...
	// Page object name
	var $PageObjName = 'offers_offer_image_path_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('offers', 'x_offer_image_path');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('offer_image_path');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('offer_image_path')));
				if (trim(strval($this->Recordset->fields('offer_image_path'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('offer_image_path') . "\"");
				}
...
// Create page object
if (!isset($offers_offer_image_path_blobview)) $offers_offer_image_path_blobview = new coffers_offer_image_path_blobview();

// Page init
$offers_offer_image_path_blobview->Page_Init();

// Page main
$offers_offer_image_path_blobview->Page_Main();
?>
<?php
$offers_offer_image_path_blobview->Page_Terminate();
?>
