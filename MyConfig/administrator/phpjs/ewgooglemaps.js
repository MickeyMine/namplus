...
function ew_ShowGoogleMapByAddress(id, maptype, address, zoom, title, desc) {
	var $ = jQuery, geocoder = new google.maps.Geocoder();
	if (address && $.trim(address) != "") {
		geocoder.geocode( {'address': address}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK) {
				...
				$("#"+id).html("Geocode was not successful for the address: '" + ew_HtmlEncode(address) + "' for the following reason: " + status);
			}
		});
	}
}
...
function ew_ShowGoogleMap(id, maptype, latlng, zoom, title, desc) {
	...
	jQuery.each(ewGoogleMaps, function(i, m) {
		if (m["id"] == id) {
			m["map"] = map;
			return false;
		}
